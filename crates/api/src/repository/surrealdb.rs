...
impl SurrealCollectionRepository {
    pub async fn try_from_env() -> anyhow::Result<Self> {
        ...
        init_auth_schema(&db).await?;

        Ok(Self { db })
    }
}

pub async fn init_auth_schema(db: &Surreal<Client>) -> anyhow::Result<()> {
    db.query(
        "
        ...
        DEFINE FIELD IF NOT EXISTS pass ON user TYPE string;
        DEFINE INDEX IF NOT EXISTS idx_user_email ON user FIELDS email UNIQUE;
        ",
    )
    .await?
    .check()?;
    println!("Auth schema initialized");
    Ok(())
}
