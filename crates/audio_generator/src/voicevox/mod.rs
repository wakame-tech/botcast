...
impl AudioGenerator for VoiceVoxClient {
    ...
    async fn generate(
        ...
        let Section::SerifSection(serif) = section else {
            return Err(anyhow::anyhow!("Invalid section type"));
        };
        let mut path_and_texts = vec![];

        for sentence in split_text(&serif.text, 100) {
            ...
            *i += 1;
            ...
            let query = self.query(&sentence, &serif.speaker).await?;
            let audio = self.synthesis(query, &serif.speaker).await?;
