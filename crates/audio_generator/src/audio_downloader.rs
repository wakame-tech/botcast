...
impl AudioGenerator for AudioDownloader {
    async fn generate(
        ...
        let Section::AudioSection(audio) = section else {
            return Err(anyhow::anyhow!("Invalid section type"));
        };
        let response = self.client.get(audio.url).send().await?;
        let bin = response.bytes().await?;
        let audio_file_path = work_dir.dir().join("tmp.mp3");
        std::fs::write(&audio_file_path, &bin)?;
