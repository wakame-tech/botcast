...
function Episode() {
  ...
  return (
    <div>
      <p className="text-xl">{episode.title}<span className="text-gray text-sm">{episode.user ? `@${episode.user.name}` : ''}</span></p>
      ...
      {episode.audio_url &&
        <div className="absolute sticky bottom-0 w-full h-15 bg-teal-100">
          <div className='p-2 flex justify-items-center'>
            <button className='m-auto' type='button' onClick={() => play()}>{isPlaying ? 'pause' : 'play'}</button>
          </div>
