...
function isImageInfo(
	val: unknown,
): val is { url: string; content_type: string; pointer: string } {
	return (
		typeof val === "object" &&
		val !== null &&
		"url" in val &&
...
function ImageFromApi({ url }: { url: string }) {
	...
	useEffect(() => {
		...
		imagesApi
			.get(collectionId, recordId, fieldName)
			.then((res) => {
	...
	return (
		<div className="edge-card">
			<div className="edge-card-header">
				...
				<span className="edge-card-target mono-text">{relation.record.id}</span>
			</div>
			<div className="edge-card-body">
...
export function CollectionDetailPage() {
	...
	const handleCreate = async (e: React.FormEvent) => {
		...
		try {
			...
			Object.values(createImagePreviews).forEach((url) =>
										...
										{createImagePreviews[fieldName] ? (
											<div className="image-field-preview">
												...
												<button
													...
													className="btn btn-ghost btn-sm"
													onClick={() =>
														handleCreateImageSelect(fieldName, null)
													}
													disabled={creating}
												>
													クリア
				...
				{records.map((r) => (
					<div key={r.id} className="card record-card">
						...
						{copyRecordId === r.id && (
							<div className="canvas-panel">
								<div className="canvas-panel-header">
									<span className="canvas-panel-title">レコードをコピー</span>
									<button
										className="btn btn-ghost btn-sm"
						...
						{splitRecordId === r.id && (
							<div className="canvas-panel">
								...
								<div className="form-group" style={{ marginTop: "0.5rem" }}>
									<label className="form-label">分割データ（JSON配列）</label>
									<textarea
										className="form-textarea"
										value={splitData}
